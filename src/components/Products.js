...
function Products() {
    ...
    useEffect(() => {
        async function getProducts() {
            await axios.get(dynamoAPI + 'getcategoryname/' + param.id).then(response => {
                setCategoryName(response.data.data);
            });
            await axios.get(dynamoAPI + 'getproductsbycategory/' + param.id).then(response => {
        ...
        <>
            <Container>
                {(products.length < 1) ?
                    <Row>
                        <p></p>
                        <h3>No products available in category {categoryName}</h3>
                    </Row>
                    :
                    <>
                        <Row>
